...
func TestProgram_BasicOutput(t *testing.T) {
	...
	model := tui.NewViewModel(getFilteredQuery, calculateStatsQuery, time.UTC, nil, 0)
...
func TestViewModel_KeyboardInteractions(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			// Create ViewModel
			vm := tui.NewViewModel(getFilteredQuery, calculateStatsQuery, time.UTC, tc.block, tc.tokenLimit)
...
func TestViewModel_LayoutResponsiveness(t *testing.T) {
	...
	vm := tui.NewViewModel(getFilteredQuery, calculateStatsQuery, time.UTC, nil, 0)
	...
	for _, size := range windowSizes {
		t.Run(size.name, func(t *testing.T) {
			...
			// Verify the view model updated its dimensions
			if vm.Width() != size.width {
				t.Errorf("Expected width %d, got %d", size.width, vm.Width())
			}
			if vm.Height() != size.height {
				t.Errorf("Expected height %d, got %d", size.height, vm.Height())
			}
...
func TestViewModel_BlockTracking(t *testing.T) {
	...
	block := CreateTestBlock()
	vm := tui.NewViewModel(getFilteredQuery, calculateStatsQuery, time.UTC, block, 7000)
...
func TestHelperFunctions(t *testing.T) {
	...
	t.Run("ReverseRequests", func(t *testing.T) {
		requests := []interface{}{"a", "b", "c", "d"}
		original := make([]interface{}, len(requests))
		copy(original, requests)
		
		tui.ReverseRequests(requests)
		
		// Verify it was reversed
		if len(requests) > 0 && requests[0] == original[0] {
			t.Errorf("ReverseRequests did not reverse the slice")
		}
	})
}
...
func TestProgram_BlockFilterInteraction(t *testing.T) {
	...
	block := CreateTestBlock()
	model := tui.NewViewModel(getFilteredQuery, calculateStatsQuery, time.UTC, block, 7000)
