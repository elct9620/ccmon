...
#### Today's Changes (Commits: `1fcbdd3`, `a024404`)
- `entity/period.go` - Fixed timezone calculation bug in `NewPeriodFromDurationWithTimezone`
- `handler/tui/model.go` - Added dual stats system, `GetStatsQuery` integration, separated display from progress stats
- `handler/tui/monitor.go` - Updated to accept `GetStatsQuery` usecase parameter
- `handler/tui/ui.go` - Updated progress bar to use `blockStats` instead of `stats`
